           ...
           <NavLink to='/'>
            ...
            <div className="hidden sm:flex items-center gap-8">
                ...
                <div className="relative cursor-pointer" onClick={()=>navigate('/cart')}>
                   <img src={assets.nav_cart_icon} alt='cart-img' className='w-6 opacity-80'/>
                    <button className="absolute -top-2 -right-3 text-xs text-white bg-primary w-[18px] h-[18px] rounded-full">{getCartCount()}</button>
                </div>
                
                {!user ? (
                    <button className="cursor-pointer px-8 py-2 bg-primary hover:bg-primary-dull transition text-white rounded-full"  onClick={()=>{ setOpen(false); setShowUserLogin(true);}}>
                    Login
                </button>
                ):(
                    ...
                    <ul className='hidden group-hover:block absolute top-10 right-0 bg-white shadow border border-gray-200 py-2.5 w-30 rounded-md text-sm z-40'>
                        <li onClick={()=>navigate('/my-orders')} className='p-1.5 cursor-pointer pl-3 hover:bg-primary/10'>My Orders</li>
                        <li onClick={logout}  className='p-1.5 cursor-pointer pl-3 hover:bg-primary/10'>Logout</li>
                    </ul>
                    </div>
                )}
